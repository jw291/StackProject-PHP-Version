...
if(isset($bno)){
	$sql='select threadname, b_title, b_content, b_id from board_advertisement where b_no='.$bno;
			...
			<!--&bno가 존재한다면 b_id를 뽑아온다. 존재하지 않으면 인풋으로 입력할 수 있게 한다 하지만 수정할때 아이디를 바꿀 필요는 없으니 사실상 무쓸모-->
				<table id="boardWrite">
					<tbody>
						...
						<tr>
							<th scope="row"><label for="threadname">모임 이름</label></th>
							<td class="title"><input type="text" name="threadname" id="threadname" value="<?php echo isset($row['threadname'])?$row['threadname']:null?>"></td>
						</tr>
						<tr>
