...
/* 페이징 시작 */
	...
	$sql = "select count(*) as cnt from board_thread ". $searchSql . "where threadname='$_SESSION[jointhreadname]'";
...
<?php
	...
	if($page != 1) {
		$paging .= '<li class="page page_start"><a href="./mythread.php?page=1' . $subString . '">처음</a></li>';
	}
	//첫 섹션이 아니라면 이전 버튼을 생성
	if($currentSection != 1) {
		$paging .= '<li class="page page_prev"><a href="./mythread.php?page=' . $prevPage . $subString . '">이전</a></li>';
	}
	...
	for($i = $firstPage; $i <= $lastPage; $i++) {
		if($i == $page) {
			...
			$paging .= '<li class="page"><a href="./mythread.php?page=' . $i . $subString . '">' . $i . '</a></li>';
		}
	}

	//마지막 섹션이 아니라면 다음 버튼을 생성
	if($currentSection != $allSection) {
		$paging .= '<li class="page page_next"><a href="./mythread.php?page=' . $nextPage . $subString . '">다음</a></li>';
	}

	//마지막 페이지가 아니라면 끝 버튼을 생성
	if($page != $allPage) {
		$paging .= '<li class="page page_end"><a href="./mythread.php?page=' . $allPage . $subString . '">끝</a></li>';
	}
	$paging .= '</ul>';
	...
	$sql = 'select * from board_thread ' . $searchSql . "where threadname = '$_SESSION[jointhreadname]' order by b_no desc" . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
...
<?
		$sql10 = "select t_master from thread where t_user= '$_SESSION[user_name]' limit 1";
		$result10 = mysqli_query($mysqli,$sql10);
		$row10 = $result10->fetch_assoc();
?>
            <div style="width:500px; height:150px; border:1px; float:left;">스레드 이름: <?php echo $_SESSION['jointhreadname'] ?> <br> 스레드 부장 이름: <?php echo $row10['t_master'] ?></div>
	    <br>
              <br>
      ...
      <!-- 스레드 게시판 table-->
		...
		<!--데이터베이스 넣기--->
							...
							<?php
							//댓글 갯수 뽑아오기
							$cosql = 'select count(*) as cnt from comment_thread where b_no = '.$row['b_no'];
							...
							<!--write/view.php?bno에서 마지막 bno는 view.php에서 GET으로 받는다. 그 bno값은 row['b_no']이다.-->
														<td class="title">
								<a href="write_thread/view.php?bno=<?php echo $row['b_no']?>"><?php echo $row['b_title']?><?php if($corow['cnt'] > 0){?><?php echo '('.$corow['cnt'].')' ?></a>
								<?php } ?>
							<?
							if($row['b_date'] == $time){
echo "<img src='./images-4.png' width='15' heigh='15'/>";
}
?>
								</a>
								    </td>
				...
				<div class="btnSet">
					...
					?>
					<a class="btnWrite btn" href="write_thread/write.php?threadname=<?php echo $_SESSION['jointhreadname']?>">글쓰기</a>
