...
if(!$ret){
	...
	if(empty($w) || $w === 'w') { //$w 변수가 비어있거나 w인 경우
		$msg = '작성';
		$sql = 'insert into comment_advertisement values(null, ' .$bno . ', ' . $coNo . ', "' . $coContent . '", "' . $coId . '")';

		if(empty($w)) { //$w 변수가 비어있다면,
			$result = $mysqli->query($sql);

			$coNo = $mysqli->insert_id;
			$sql = 'update comment_advertisement set co_order = co_no where co_no = ' . $coNo;
		}

	} else if($w === 'u') { //작성
		$msg = '수정';

		$sql = 'select count(*) as cnt from comment_advertisement where co_no = ' . $coNo;
		$result = $mysqli->query($sql);
...
<?php
		...
		$sql = 'update comment_advertisement set co_content = "' . $coContent . '" where co_no = ' . $coNo;
...
<?php
		...
		$sql = 'delete from comment_advertisement where co_no = ' . $coNo;
