...
// -- 로그인 안한상태에서 글쓰기 버튼 클릭 시 alert 띄우기 --
	...
	<!-- 게시판 -->
				<li class="nav-item" data-toggle="tooltip" data-placement="right" title="Board">
 				...
 				<ul class="sidenav-second-level collapse" id="collapseboard">
					...
					<li>
	 					<a href="notice-table.php">자유 게시판</a>
                    </li>
                    <li>
		...
		?>
				<li class="nav-item" data-toggle="tooltip" data-placement="right" title="Components">
					...
					<ul class="sidenav-second-level collapse" id="collapseComponents">
						<li>
<?
$sql11 = "select jointhreadname from user where user_id= '$_SESSION[user_id]'";
$result11 = mysqli_query($mysqli,$sql11);
$row11 = mysqli_fetch_assoc($result11);
$_SESSION['jointhreadname'] = $row11['jointhreadname'];
?>
							<a href="mythread.php?threadname=<?php echo $_SESSION['jointhreadname'] ?>">나의 스레드</a>
						</li>
						<li>
							<a href="thread_construct.php">스레드 개설하기</a>
						</li>
					</ul>
			</li>
...
<?php
//댓글 갯수 뽑아오기
$cosql = 'select count(*) as cnt from comment_free where b_no = '.$row['b_no'];
...
?>
		...
		<!--write/view.php?bno에서 마지막 bno는 view.php에서 GET으로 받는다. 그 bno값은 row['b_no']이다.-->
                  <td class="title">
			<a href="write_free/view.php?bno=<?php echo $row['b_no']?>"><?php echo $row['b_title']?><?php if($corow['cnt'] > 0){?><?php echo '('.$corow['cnt'].')'?></a>
			<?php } ?>
<?
if($row['b_date'] == $time){
...
?>
		...
		?>            
									...
									?>
									<a class="btnWrite btn" href="write_free/write.php">글쓰기</a>
